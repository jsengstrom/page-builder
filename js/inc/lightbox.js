        ...
        var plugin = {
            ...
            init: function (items) {
                ...
                if (!plugin.lightbox) {
                    ...
                    plugin.lightbox = jQuery("#lightbox");
                    plugin.caption = jQuery('.lightbox-caption', plugin.lightbox);
                }

                if (plugin.items.length > 1 && opts.nav) {
                    jQuery('.lightbox-nav', plugin.lightbox).show();
                } else {
                    jQuery('.lightbox-nav', plugin.lightbox).hide();
                }
            ...
            loadImage: function () {
                ...
                jQuery("img", plugin.lightbox).remove();
                plugin.lightbox.fadeIn('fast').append('<span class="lightbox-loading"></span>');

                var img = jQuery('<img src="' + jQuery(plugin.current).attr('href') + '" draggable="false">');

                jQuery(img).load(function () {
                    jQuery('.lightbox-loading').remove();
                    plugin.lightbox.append(img);
                    plugin.image = jQuery("img", plugin.lightbox).hide();
                    plugin.resizeImage();
                    plugin.setCaption();
                });
            },
            ...
            setCaption: function () {
                var caption = jQuery(plugin.current).data('caption');
            ...
            bindEvents: function () {
                jQuery(plugin.items).click(function (e) {
                    if(!jQuery("#lightbox").is(":visible") && (jQuery(window).width() < opts.minSize || jQuery(window).height() < opts.minSize)) {
                ...
                // Previous click
                jQuery(plugin.lightbox).on('click', '.lightbox-previous', function () {
                    plugin.previous();
                    return false;
                });

                // Next click
                jQuery(plugin.lightbox).on('click', '.lightbox-next', function () {
                    plugin.next();
                    return false;
                });

                // Close click
                jQuery(plugin.lightbox).on('click', '.lightbox-close', function () {
                    plugin.close();
                    return false;
                });
            ...
            close: function () {
                ...
                jQuery(plugin.lightbox).fadeOut('fast');
